...
public class BaseTest extends SeleneseTestCase {
	...
	public BaseTest() {
		try {
			...
			url = Utils.getConfigEntry(f, "urlteams");
			browser = Utils.getConfigEntry(f, "browser");
			speed = Utils.getConfigEntry(f, "speed");
			path = Utils.getConfigEntry(f, "path");
			username = Utils.getConfigEntry(f, "username");
			username2 = Utils.getConfigEntry(f, "username2");
			password = Utils.getConfigEntry(f, "password");
		}
		catch (Exception e) {
			e.printStackTrace();
		}
